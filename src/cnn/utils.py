...
def load_image(image_path, target_size=(224, 224)):
    """
    Load an image from a file path, resize it, and normalize it to [0, 1].
    
    Args:
        image_path (str): Path to the image file.
        target_size (tuple): The target size for resizing (height, width).
        
    Returns:
        np.ndarray: The processed image array with shape (target_size[0], target_size[1], 3).
    """
...
def load_batch(image_paths, target_size=(224, 224)):
    """
    Load and process a batch of images from a list of paths.
    ...
    Returns:
        np.ndarray: A numpy array of shape (N, H, W, C).
    """
    batch_images = []
...
def extract_and_save_features(model, image_paths, target_size, save_path, batch_size=32):
    """
    Use a Keras CNN model to extract features from a list of images and save them.
    
    Args:
        model (keras.Model): The pretrained CNN model (frozen).
        image_paths (list): List of paths to the image files.
        target_size (tuple): The target size for the CNN input.
