...
from captioning.rnn.layers import EmbeddingLayer, StackedRNNCell
...
class RNNCaptioner:
    def __init__(self, vocab_size, embed_dim, hidden_dim, max_length, num_layers=1):
        ...
        self.stacked_rnn = StackedRNNCell(embed_dim, hidden_dim, num_layers)
    ...
    def generate_caption_beam(self, image_features, tokenizer, beam_width=3, max_length=None):
        ...
        h_list_init = [np.zeros((1, self.hidden_dim)) for _ in range(self.num_layers)]
        
        h_list_init = self.stacked_rnn.forward(img_embed, h_list_init)
        
        start_token = tokenizer.word_index[tokenizer.start_token]
        beam = [(0.0, [start_token], h_list_init, False)]
        final_sequences = []
        ...
        for step in range(max_length):
            ...
            for log_prob, tokens, state, _ in beam:
                if len(tokens) > 0:
                    ...
                    h_list_cp = [h.copy() for h in state]
                    h_list_new = self.stacked_rnn.forward(x_input, h_list_cp)
                    
                    preds = self.output_layer.forward(h_list_new[-1])[0]
                    ...
                    for next_token_idx in top_k_indices:
                        ...
                        candidates.append((new_log_prob, new_tokens, h_list_new, is_end))
